...
const EditFaq = () => {
  ...
  useEffect(() => {
    ...
    if (layoutSuccess) {
      toast.success("FAQ updated succssfully");
    }
    if (error) {
      if ("data" in error) {
          ...
          {questions?.map((q: any) => (
            ...
            >
              <dt className="text-lg">
                <button
                  className="flex items-center text-white justify-between w-full text-left focus:outline-none"
                ...
                >
                  ...
                  />
                  <span className="ml-6 flex-shirnk-0">
                    {q.active ? (
